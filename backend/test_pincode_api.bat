...
REM Test 1: Valid Bangalore pincode (560001)
...
curl -X PUT "http://localhost:8080/auth/profile?email=admin@petco.com" ^
  ...
  -d "{\"name\": \"Admin User\", \"pincode\": \"560001\"}" ^
  -w "\nStatus Code: %%{http_code}\n\n"

REM Test 2: Valid Bangalore pincode (560454)  
...
curl -X PUT "http://localhost:8080/auth/profile?email=nishu@gmail.com" ^
  -H "Content-Type: application/json" ^
  -d "{\"name\": \"Nishu\", \"pincode\": \"560454\"}" ^
  -w "\nStatus Code: %%{http_code}\n\n"

REM Test 3: Invalid pincode (not Bangalore - 110001)
echo Test 3: Invalid pincode (110001 - Delhi) for nishu@gmail.com
curl -X PUT "http://localhost:8080/auth/profile?email=nishu@gmail.com" ^
  -H "Content-Type: application/json" ^
  -d "{\"name\": \"Nishu\", \"pincode\": \"110001\"}" ^
  -w "\nStatus Code: %%{http_code}\n\n"

REM Test 4: Invalid pincode format (12345)
echo Test 4: Invalid pincode format (12345) for nishu@gmail.com
curl -X PUT "http://localhost:8080/auth/profile?email=nishu@gmail.com" ^
  -H "Content-Type: application/json" ^
  -d "{\"name\": \"Nishu\", \"pincode\": \"12345\"}" ^
  -w "\nStatus Code: %%{http_code}\n\n"

REM Test 5: Get updated profile to verify save
