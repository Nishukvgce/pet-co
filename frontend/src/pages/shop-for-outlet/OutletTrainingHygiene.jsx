...
const OutletTrainingHygiene = ({ initialActive = 'All Training & Hygiene' }) => {
  ...
  const sections = getFilterSections('outlet', 'training');
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        console.error('Outlet Training: load failed', err);
        setProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    ...
    if (active !== 'All Training & Hygiene') {
      working = working.filter(p => !active || p.subcategory === active || (p.category === active));
       ...
       // Filter for training/hygiene generally
       working = working.filter(p => {
          const cat = (p.category || '').toLowerCase();
          const sub = (p.subcategory || '').toLowerCase();
          return cat.includes('training') || cat.includes('hygiene') || cat.includes('cleaning') || sub.includes('training') || sub.includes('hygiene');
       });
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Outlet Training & Hygiene - Pet & Co</title>
      </Helmet>
      <Header />
      ...
      <div className="container mx-auto px-4 py-6 pb-24">
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Essential products for training and keeping your pet clean.
          </p>
        </div>
