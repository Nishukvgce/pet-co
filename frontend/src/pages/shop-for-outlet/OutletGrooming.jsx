...
const OutletGrooming = ({ initialActive = 'All Grooming & Care' }) => {
  ...
  const sections = getFilterSections('outlet', 'grooming');
  ...
  useEffect(() => {
    ...
    if (active !== 'All Grooming & Care') {
      working = working.filter(p => !active || p.subcategory === active || (p.category === active));
       ...
       // Filter for grooming generally
       working = working.filter(p => {
          const cat = (p.category || '').toLowerCase();
          const sub = (p.subcategory || '').toLowerCase();
          return cat.includes('groom') || cat.includes('care') || cat.includes('shampoo') || sub.includes('groom') || sub.includes('care');
       });
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Outlet Grooming & Care - Pet & Co</title>
      </Helmet>
      <Header />
      ...
      <div className="container mx-auto px-4 py-6 pb-24">
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Discounted grooming essentials to keep your pets looking their best.
          </p>
        </div>
              ...
              <div className="flex items-center justify-center h-64 bg-white rounded-lg border border-border">
               ...
               <div className="flex items-center justify-center h-64 bg-white rounded-lg border border-border">
                 <div className="text-center">
                   ...
                   <div className="text-foreground font-semibold mb-2">No grooming items found in "{active}"</div>
                 </div>
               </div>
            ) : (
