...
const OutletWalkingEssentials = ({ initialActive = 'All Walking Essentials' }) => {
  ...
  const sections = getFilterSections('outlet', 'walking');
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        // Normalize API product objects for consistent fields
        ...
        console.error('Outlet Walking: load failed', err);
        setProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    ...
    if (active !== 'All Walking Essentials') {
      working = working.filter(p => !active || p.subcategory === active || (p.category === active));
       ...
       // Filter for walking essentials generally
       working = working.filter(p => {
          const cat = (p.category || '').toLowerCase();
          const sub = (p.subcategory || '').toLowerCase();
          return cat.includes('walking') || cat.includes('leash') || cat.includes('collar') || sub.includes('walking') || sub.includes('leash');
       });
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Outlet Walking Essentials - Pet & Co</title>
      </Helmet>
      <Header />
      ...
      <div className="container mx-auto px-4 py-6 pb-24">
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Discounted collars, leashes, and harnesses for your pets.
          </p>
        </div>
