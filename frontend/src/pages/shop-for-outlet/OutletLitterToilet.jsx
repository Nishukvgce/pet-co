...
const categories = [
  { id: 'cat-litter', label: 'Cat Litter', img: '/assets/images/outlet/cat-litter.png' },
  { id: 'litter-boxes', label: 'Litter Boxes', img: '/assets/images/outlet/litter-boxes.png' },
  { id: 'scoops-mats', label: 'Scoops & Mats', img: '/assets/images/outlet/scoops.png' },
  { id: 'cleaning-disposal', label: 'Cleaning & Disposal', img: '/assets/images/outlet/cleaning.png' },
  { id: 'all', label: 'All Litter & Toilet', img: '/assets/images/outlet/cat-litter.png' }
];

const OutletLitterToilet = ({ initialActive = 'All Litter & Toilet' }) => {
  ...
  const topFilters = ['Brand', 'Life Stage', 'Price', 'Weight'];
  const [selectedTopFilter, setSelectedTopFilter] = useState(topFilters[0]);
  const [selectedFilters, setSelectedFilters] = useState({});
  const [appliedFilters, setAppliedFilters] = useState({});

  const sections = getFilterSections('outlet', 'litter');
  ...
  const openFilterAndScroll = (key) => {
    ...
    setFilterOpen(true);
    // Map 'Brand' -> 'brands', 'Life Stage' -> 'lifeStages', etc. if needed
    // But sections.id usually matches. Here we rely on sections having matching labels or IDs?
    // Actually sections have IDs like 'brands', but topFilters are labels 'Brand'.
    // We need to find the section ID corresponding to the label.
    const sectionId = sections.find(s => s.label === key)?.id || key.toLowerCase();
    
    // Slight delay to allow drawer to open
    setTimeout(() => {
      // We can pass the section ID to the drawer or scroll to it if we had refs inside the drawer...
      // But FilterDrawer handles its own rendering. We can just open it.
      // If we want to scroll to specific section in FilterDrawer, we'd need to modify FilterDrawer or use a ref prop.
      // For now, just opening is enough as per other implementations which might not be perfectly scrolling yet outside of this file context.
      // Wait, in previous files I implemented manual drawer. Here I am using FilterDrawer component.
      // The previous files like OutletTrainingHygiene seem to use FilterDrawer component too (I copied it back in).
      // Oh wait, OutletTrainingHygiene used FilterDrawer component but I replaced it with manual drawer in my previous thought block?
      // No, I replaced it with FilterDrawer component usage in the actual code replacement.
      // So I don't need manual scroll logic for FilterDrawer component unless it supports it.
      // I will just open it.
    }, 0);
  };
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        console.error('Outlet Litter: load failed', err);
        setProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    ...
    // Category filter
    if (active !== 'All Litter & Toilet') {
      working = working.filter(p => !active || p.subcategory === active || (p.category === active));
    } else {
       // Filter for litter/toilet generally
       working = working.filter(p => {
          const cat = (p.category || '').toUpperCase();
          const sub = (p.subcategory || '').toUpperCase();
          return cat.includes('LITTER') || cat.includes('TOILET') || sub.includes('LITTER') || sub.includes('TOILET');
       });
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Outlet Litter & Toilet - Pet & Co</title>
      </Helmet>
      <Header />
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
                ...
                {filteredProducts.length === 0 && (
                   <div className="col-span-full text-center py-10 text-muted-foreground">
                     ...
                     No litter or toilet products found in "{active}"
                   </div>
                )}
              </div>
            )}
