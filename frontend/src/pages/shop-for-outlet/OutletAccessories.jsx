...
const OutletAccessories = ({ initialActive = 'All Accessories' }) => {
  ...
  useEffect(() => {
    ...
    if (active !== 'All Accessories') {
      working = working.filter(p => !active || p.subcategory === active || (p.category === active));
       ...
       // Filter for accessories generally
       working = working.filter(p => {
          const cat = (p.category || '').toLowerCase();
          const sub = (p.subcategory || '').toLowerCase();
          return cat.includes('accessor') || cat.includes('training') || cat.includes('hygiene') || sub.includes('accessor');
       });
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Outlet Accessories - Pet & Co</title>
      </Helmet>
      <Header />
