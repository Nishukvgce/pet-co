...
const OutletTravelSafety = ({ initialActive = 'All Travel & Safety' }) => {
  ...
  const sections = getFilterSections('outlet', 'travel');
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        console.error('Outlet Travel: load failed', err);
        setProducts([]);
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    ...
    if (active !== 'All Travel & Safety') {
      working = working.filter(p => !active || p.subcategory === active || (p.category === active));
       ...
       // Filter for travel generally
       working = working.filter(p => {
          const cat = (p.category || '').toLowerCase();
          const sub = (p.subcategory || '').toLowerCase();
          return cat.includes('travel') || cat.includes('carrier') || cat.includes('safety') || sub.includes('travel') || sub.includes('carrier');
       });
    }
  ...
  return (
    <div className="min-h-screen bg-background">
      <Helmet>
        <title>Outlet Travel & Safety - Pet & Co</title>
      </Helmet>
      <Header />
      ...
      <div className="container mx-auto px-4 py-6 pb-24">
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Safe and comfortable travel solutions for your pets.
          </p>
        </div>
              ...
              <div className="flex items-center justify-center h-64 bg-white rounded-lg border border-border">
               ...
               <div className="flex items-center justify-center h-64 bg-white rounded-lg border border-border">
                 <div className="text-center">
                   ...
                   <div className="text-foreground font-semibold mb-2">No travel items found in "{active}"</div>
                 </div>
               </div>
            ) : (
