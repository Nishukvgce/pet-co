...
const CatCollars = ({ initialActive = 'All Collars & Accessories' }) => {
  ...
  useEffect(() => {
    ...
    if (selectedFilters.material?.length>0) working = working.filter(p => selectedFilters.material.some(m => String(p.material || '').toLowerCase().includes(m.toLowerCase()) || (p.tags||[]).some(t=>t.toLowerCase().includes(m.toLowerCase()))));
    if (selectedFilters.sizes?.length>0) working = working.filter(p => selectedFilters.sizes.some(s => String(p.size||'').toLowerCase().includes(s.toLowerCase()) || (p.variants||[]).some(v=>String(v).toLowerCase().includes(s.toLowerCase()))));
    if (selectedFilters.purpose?.length>0) working = working.filter(p => selectedFilters.purpose.some(pu => String(p.productType||'').toLowerCase().includes(pu.toLowerCase()) || (p.tags||[]).some(t=>t.toLowerCase().includes(pu.toLowerCase()))));
  ...
  <div ref={drawerContentRef} className="px-4 pt-4 pb-32 hide-scrollbar overflow-y-auto" style={{ maxHeight: 'calc(100vh - 140px)' }}>
            ...
            <div className="flex flex-wrap gap-2">
              {material.map(m=> (
                <button 
                  ...
                  className={`text-xs px-3 py-1 border border-border rounded ${selectedFilters.material.includes(m) ? 'bg-orange-500 text-white border-orange-500' : 'bg-white'}`}
                >
                  {m}
                </button>
              ))}
             ...
             <div className="flex flex-wrap gap-2">
               {purpose.map(p=> (
                 <button 
                   ...
                   className={`text-xs px-3 py-1 border border-border rounded ${selectedFilters.purpose.includes(p) ? 'bg-orange-500 text-white border-orange-500' : 'bg-white'}`}
                 >
                   {p}
                 </button>
               ))}
        ...
        <div className="fixed bottom-0 right-0 left-auto w-full sm:w-96 bg-white border-t p-4 flex items-center justify-between">
          <button onClick={() => setSelectedFilters({ brands: [], material: [], sizes: [], purpose: [], priceRanges: [], subCategories: [], catKitten: [], lifeStages: [], breedSizes: [], productTypes: [], specialDiets: [], proteinSource: [], weights: [], sortBy: '' })} className="text-sm text-orange-500">Clear All</button>
